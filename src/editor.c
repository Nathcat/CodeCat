    ...
    GtkBox *fileTree)
{
    ...
    g_signal_connect(ccat_key_event_controller, "key-pressed", G_CALLBACK(ccat_editor_key_press_event), NULL);
    ...
    gpointer user_data)
{
    ...
    // Also set the title of the window to indicate that there is unsaved content in the file
    if (ccat_active_editor.filePath != 0) gtk_window_set_title(ccat_main_window, "CodeCat - *Unsaved*");
    else gtk_window_set_title(ccat_main_window, "CodeCat - No file open");
}
    ...
    gpointer user_data)
{
    ...
    if (ccat_active_editor.__tabbing)
        return;
...
void ccat_editor_load(GtkWindow *target)
{
    ...
    gtk_window_set_title(ccat_main_window, "CodeCat - No file open");
}
...
void ccat_editor_open_file(GtkButton *target)
{
    ...
    ccat_active_editor.filePath = filename;
    ccat_active_editor.__tabbing = true;
    ...
    fclose(ccat_active_editor.file);

    ccat_active_editor.__tabbing = false;

    printf("Done\n");
    gtk_window_set_title(ccat_main_window, "CodeCat - Saved");
}
    ...
    gpointer user_data)
{
    ...
    if (ccat_active_editor.filePath == 0) 
        return false;
    ...
    const char *data = gtk_text_buffer_get_text(
        buffer,
        start,
        end,
        false);
    
    ccat_active_editor.file = fopen(ccat_active_editor.filePath, "w");
    fputs(data, ccat_active_editor.file);
    fclose(ccat_active_editor.file);

    printf("Written %d bytes to %s\n", strlen(data) * sizeof(char), ccat_active_editor.filePath);
    gtk_window_set_title(ccat_main_window, "CodeCat - Saved");
    return true;
}
    ...
    gpointer user_data)
{
    switch (keyval) {
        case GDK_KEY_s:
            printf("Key sequence is CTRL+S\n");
            if (state == GDK_CONTROL_MASK)
                return ccat_editor_auto_save(self, keyval, keycode, state, user_data);
            break;
        
        default:
            printf("Key sequence not special\n");
            return false;
    }
}